...
describe('SnakeGame ADT', () => {
    describe('create', () => {
        test('creates a new game with default dimensions', () => {
            ...
            expect(state.inputQueue.length).toBe(0);
        });
    ...
    describe('queueDirection', () => {
        test('queues a valid direction', () => {
            const game = SnakeGame.create().start();
            const updated = game.queueDirection('UP');
            const state = updated.serialize();

            expect(state.inputQueue.length).toBe(1);
            expect(state.inputQueue[0]).toBe('UP');
        });

        test('queues up to 2 directions', () => {
            const game = SnakeGame.create().start();
            const updated = game.queueDirection('UP').queueDirection('LEFT');
            const state = updated.serialize();

            expect(state.inputQueue.length).toBe(2);
            expect(state.inputQueue[0]).toBe('UP');
            expect(state.inputQueue[1]).toBe('LEFT');
        });

        test('ignores third direction when queue is full', () => {
            const game = SnakeGame.create().start();
            const updated = game
                .queueDirection('UP')
                .queueDirection('LEFT')
        ...
        test('does not queue when game over', () => {
            // Create a game that will hit wall immediately
            const game = SnakeGame.create(3, 3).start();
            // Move left into wall (snake starts at center facing RIGHT, move left to hit edge faster)
            const updated = game.queueDirection('LEFT');
            ...
            expect(afterQueue.serialize().inputQueue.length).toBe(0);
        });
    });
    ...
    describe('tick', () => {
        ...
        test('processes direction from input queue', () => {
            ...
            expect(state2.inputQueue.length).toBe(0);
        });
    ...
    describe('serialize', () => {
        test('returns complete game state', () => {
            ...
            expect(state).toHaveProperty('inputQueue');
    ...
    describe('invariant checking', () => {
        ...
        test('maintains input queue size limit', () => {
            const game = SnakeGame.create().start();
            const updated = game
                .queueDirection('UP')
                .queueDirection('LEFT')
